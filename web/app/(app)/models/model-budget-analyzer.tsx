...
function CapabilityCostGraphic({
  rows,
}: {
  rows: Array<{ model: ModelProfile; monthlyCost: number }>;
}) {
        ...
        <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Two normalized bars per model make the trade-off visible at a glance.
            </p>
          </div>
          ...
          {rows.map(({ model, monthlyCost }) => (
            <div key={model.model} className="grid gap-2 md:grid-cols-[11rem_1fr_1fr_5rem] md:items-center">
              <div className="min-w-0">
                ...
                <p className="text-[11px] capitalize text-muted-foreground">{model.provider} · {model.tier}</p>
              </div>
              <div>
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        {rows.map(({ model, monthlyCost }) => (
          <Card
            key={model.model}
            className={`border-border/80 shadow-none ${activeModel === model.model ? "ring-1 ring-primary" : ""}`}
          >
            <CardContent className="pt-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Badge variant="outline" className="capitalize">{model.tier}</Badge>
                  </div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
                ...
                <div className="rounded-lg bg-secondary/55 p-2">
                  ...
                  <p className="text-sm font-semibold">${model.inputPer1M} / ${model.outputPer1M}</p>
                </div>
              </div>
      ...
      <p className="text-xs text-muted-foreground">
        This catalog intentionally lists text/agent models compatible with the Brain OS structured
        command path. Image, audio, realtime, embedding, deprecated, and invitation-only models are
        separate workloads and are not selectable as the company-command model.
      </p>
    </div>
  );
}
