// VERIFIER #51 — run every qa/scenarios-runner/*.mjs suite AND every standing verifier gate as its OWN child
// process; the recorded status is the CHILD's exit status (never a pipeline's). Prints the pass/fail tally each
// suite reports so a suite that asserts nothing is visible, and screens for zero-row generators.
...
const GATES = [
  ...
  'qa/verification/proposed/v50_regression_additions.mjs',
...
function run(kind, f) {
  ...
  const counts = out.match(/(\d+)\s+passed[,\s]+(\d+)\s+failed/i) || out.match(/PASS(?:ED)?[=: ]+(\d+)[^\d]+FAIL(?:ED)?[=: ]+(\d+)/i) || out.match(/(\d+)\s+pass(?:ed)?\s*\/\s*(\d+)\s+fail(?:ed)?/i) || out.match(/(\d+)\/(\d+)\s+(?:passed|ok|cases)/i);
  const superseded = /SUPERSEDED/i.test(out);
  const zeroGen = /\b0 (?:rows|cases|shapes|corpus rows|inputs)\b/i.test(out);
  const tail = out.trim().split('\n').slice(-2).join(' | ').slice(0, 240);
  const fails = out.split('\n').filter((l) => /^\s*(FAIL|not ok|✗|✘|\*\*\* FAIL)/i.test(l)).slice(0, 6).map((l) => l.trim().slice(0, 200));
  ...
  console.log(`${String(r.status).padStart(3)}  ${kind.padEnd(5)} ${f.padEnd(80)} ${counts ? counts[1] + '/' + counts[2] : (superseded ? 'SUPERSEDED' : 'no-count')}${zeroGen ? '  [ZERO-GEN?]' : ''}  ${Date.now() - t0}ms`);
...
console.log('suites printing no pass/fail count (vacuity screen): ' + (su.filter((r) => r.passed === null && !r.superseded).map((r) => r.file + ' :: ' + r.tail).join('\n    ') || 'none'));
...
writeFileSync('qa/verification/scratch/v51/battery.json', JSON.stringify(results, null, 2));
