// VERIFIER #52 — four-quadrant differential, candidate vs deployed v92, on my own corpus.
// Usage: node differential.mjs [--empty] [--structured] [--src <index.ts>]   (writes a .log and .json beside itself)
...
const SRC = srcIdx >= 0 ? args[srcIdx + 1] : H.SRC;
const tag = (srcIdx >= 0 ? 'mut_' + SRC.split(/[\\/]/).pop().replace(/\.ts$/, '') : 'cand') + (EMPTY ? '_empty' : '_populated') + (STRUCTURED ? '_structured' : '');
...
for (const [sec, o] of [...bySection.entries()].sort()) {
  for (const k of Object.keys(tot)) tot[k] += o[k];
  out += `${sec.padEnd(12)} ${String(o.n).padStart(4)}   ${String(o.T_bothKeep).padStart(8)} ${String(o.T_bothDestroy).padStart(11)} ${String(o.T_RESCUE).padStart(6)} ${String(o.T_REGRESSION).padStart(10)} | ${String(o.F_bothDestroy).padStart(11)} ${String(o.F_GAIN).padStart(4)} ${String(o.F_bothShip).padStart(8)} ${String(o.F_REGRESSION).padStart(10)}\n`;
}
...
const list = (q, title) => { const r = rowsOut.filter((x) => x.q === q); out += `## ${title} (${r.length})\n`; for (const x of r) out += `  [${x.section}] cand=${x.cand} v92=${x.v92} :: ${x.text}\n`; out += '\n'; };
...
list('T_bothDestroy', 'both destroy (shared cost, for the record)');
console.log(out);
