// VERIFIER #53 — run every qa/scenarios-runner/*.mjs suite and every standing verifier gate as its OWN child
// process (child exit status, never a pipeline status); print each suite's own tally; screen for vacuity.
...
const GATES = [
  ...
  'qa/verification/proposed/v51_regression_additions.mjs',
  'qa/verification/proposed/v52_regression_additions.mjs',
...
function run(kind, f) {
  ...
  const counts = out.match(/(\d+)\s+passed[,\s]+(\d+)\s+failed/i) || out.match(/PASS(?:ED)?[=: ]+(\d+)[^\d]+FAIL(?:ED)?[=: ]+(\d+)/i) || out.match(/(\d+)\s+pass(?:ed)?\s*\/\s*(\d+)\s+fail(?:ed)?/i) || out.match(/(\d+)\/(\d+)\s+(?:passed|ok|cases|contract checks)/i);
  ...
  console.log(`${String(r.status).padStart(3)}  ${kind.padEnd(5)} ${f.padEnd(84)} ${counts ? counts[1] + '/' + counts[2] : (superseded ? 'SUPERSEDED' : 'no-count')}${zeroGen ? '  [ZERO-GEN?]' : ''}  ${Date.now() - t0}ms`);
...
writeFileSync('qa/verification/scratch/v53/battery.json', JSON.stringify(results, null, 2));
