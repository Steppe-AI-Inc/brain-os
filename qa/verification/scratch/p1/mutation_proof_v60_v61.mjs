...
const MUTANTS = [
  ...
  ['M13 V61-D6 the Mongolian tier drops its verb-final requirement',
    ...
    (s) => s.replace('mnIsCommandForm(w) && mnFinalWindow.includes(w)', 'mnIsCommandForm(w)')],
  ['M14 V61-D6 the Mongolian tier drops its morphology test',
    'qa/verification/proposed/v61_regression_additions.mjs',
    (s) => s.replace('const mnIsCommandForm = (w: string) => !MN_NOT_A_COMMAND.test(w) && !MN_CASE_SUFFIX.test(w);', 'const mnIsCommandForm = (w: string) => true;')],
