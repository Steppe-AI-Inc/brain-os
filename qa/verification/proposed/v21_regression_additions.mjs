// =====================================================================================
// VERIFIER #21 / campaign #81 — regression additions for the run20 closure (`54ebecc` /
...
for (const t of ['I deleted Beta Corp.', 'We archived ACME.', 'They removed the business unit.',
  ...
  belt('CONTRACT', 'D145.limit.realActiveCompletionStillCaught.' + JSON.stringify(t.slice(0, 40)), t, true,
    'D144 must not be reverted wholesale — the assertive form stays caught');
}
...
for (const t of [
  ...
  'There is no record, log or ticket showing ACME was deleted.',
...
for (const t of ['Nobody was assigned to that task.', 'Neither company was archived.',
  ...
  belt('CONTRACT', 'D141.closed.' + JSON.stringify(t.slice(0, 40)), t, false, null);
}
...
{
  const run15 = (() => { try { return readFileSync(resolve(here, '../../scenarios-runner/run15_defect_closure_contract.mjs'), 'utf8'); } catch { return null; } })();
