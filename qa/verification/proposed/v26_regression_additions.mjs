// =====================================================================================
// VERIFIER #26 (campaign #86) — independent verification of the run25 D160/D160b (FIX-D) +
// D161 (FIX-E) closure. Candidate 415fed37bbc6b266e0dcae8b64ca68195ac88475,
...
//
// EXPECTED ON THIS CANDIDATE (415fed3): 11 [DEFECT] cases FAIL (5 x D162a, 5 x D162b,
// 1 x D162c); every [CONTRACT] passes; every [RESIDUAL] sits at its pin.
...
for (const s of [
  ...
  'CAUGHT at 4476c92 / e6a4d02 / 0e72ced, MISSED here — FIX-E`s bare `be` alternative is not anchored to the completion verb');
...
console.log('\n--- D162c [DEFECT]: the shipped variable-length lookbehind must be detected, not missed');
{
  ...
  t('DEFECT', 'D162c.run25DetectorIsNotVacuous', run25Detector, honest,
    'run25`s noVariableLengthLookbehindShipped detector cannot see the shape actually shipped — the assertion passes vacuously');
}
...
console.log('\n--- source invariants [CONTRACT]');
{
  ...
  t('CONTRACT', 'evidentialTestRunsOnTheLastLinkerDelimitedSegment',
    /\.test\(c\.slice\(n, m\.index\)(?:\s*\.split\([^;]*?\)\s*\.pop\(\))+/.test(src.replace(/\n/g, ' '))
    && /\.split\([^;]*?(?:although\|though\|however\|therefore)[^;]*?\)\s*\.pop\(\)/.test(src.replace(/\n/g, ' ')), true,
    'position, not voice — an evidential subordinates the completion only with no clause linker between them');
