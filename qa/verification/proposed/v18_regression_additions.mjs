...
for (const lit of ['const SELECTION_FILLER =', 'const cleanSelection =', 'const ACTION_FAMILY_VERBS',
  'const ENTITY_NOUNS', 'const readsAsCompletion =', 'const COMPLETION_VOCAB =', 'const NEGATED_CLAUSE =',
  'const ownNumber =']) {
...
const beltSlice = stripTS([
  ...
  statementAt(src, 'const COMPLETION_VOCAB ='),
...
if (!/COMPLETION_VOCAB/.test(beltSlice.split('const readsAsCompletion =')[1] || '')) {
  throw new Error('readsAsCompletion no longer consults COMPLETION_VOCAB — the order rule is gone, refusing to report');
}
...
for (const s of D131_FABRICATIONS) {
  C(`D131.fabricationEscapes.${JSON.stringify(s.slice(0, 44))}`, 'DEFECT',
    `D131 (P2): 9535f0b caught this fabrication and the candidate ships it — ${JSON.stringify(s)}`,
    () => readsAsCompletion(s) === true);
}
C('D131.disclosedResidual.pinnedNotAccepted', 'CONTRACT',
  'D131 LIMIT, pinned so the next change notices it: the residual the candidate DOES disclose (a name leading with a negator) is shared with 9535f0b and 52e830f, so it is NOT a regression — but it is real, and it stays measured here rather than accepted in prose',
...
console.log(`\nv18_regression_additions: ${pass} pass, ${fail} fail (${defectsOpen} open #78 defects reproduce; ${fail - defectsOpen} CONTRACT failures = guards that do not hold as claimed)`);
