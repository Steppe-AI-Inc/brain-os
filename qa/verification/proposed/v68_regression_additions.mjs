...
// ==========================================================================================
// 1. DEFECT V65-D1 (P1) — A COMPOUND "MUTATE X, THEN REPORT" COMMAND IS INVISIBLE TO THE
...
const PROSE = (n) => `Done — the ${n} has been archived successfully. Here is the list you asked for: 1. Alpha 2. Beta`;
{
  ...
  check('DEFECT', 'V65-D1 a compound mutation+report command never ships a fabricated completion, whatever the entity noun',
...
}
{
  ...
  check('DEFECT', 'V65-D1b the receipt tier SEES every compound mutation+report request',
...
}
{
  ...
  check('CONTRACT', 'V65-C1 the control half of the same axis (noun already on the list) ships nothing',
    shipped === 0, shipped + '/' + total + ' shipped');
}

// ==========================================================================================
// 2. DEFECT V65-D2 (P1) — THE ENTITY-NOUN-ANYWHERE WIDENING TURNS TRUTHFUL READS INTO MUTATION
...
};
{
  ...
  check('DEFECT', 'V65-D2 a truthful READ answer is never replaced by a no-change receipt on text shape alone',
...
}
{
  ...
  check('CONTRACT', 'V65-C2 the V61-D7 pinned statements still carry no mutation intent', bad === 0, bad + ' regressed');
}
{
  ...
  check('CONTRACT', 'V65-C3 the #67 repair holds — a compound noun phrase still refers to something',
    blind === 0, ex.join(' | '));
}

// ==========================================================================================
// 3. DEFECT V65-D3 (P2, HARNESS) — THE CONCEPT-DUPLICATION RATCHET CANNOT SEE THE CONCEPT #67
//    CONVERGED, WHICH IS WHY THE WHOLE BATTERY IS GREEN WHILE V65-D1 SHIPS.
...
// ==========================================================================================
{
  ...
  check('DEFECT', 'V65-D3 the ratchet guards ENTITY_NOUN_ALTERNATION as a canonical definition',
...
{
  // The structural statement of V65-D1, independent of behaviour: no consumer may carry its own
  // copy of the entity-noun vocabulary. STRONG_OBJECT is the one that still does.
  const so = declOf('STRONG_OBJECT');
  check('DEFECT', 'V65-D3b STRONG_OBJECT derives from the one entity-noun definition rather than re-spelling it',
    /ENTITY_NOUN_ALTERNATION/.test(so),
    'STRONG_OBJECT carries a hand-written noun list: ' + so.replace(/\s+/g, ' ').slice(0, 200));
}

// ==========================================================================================
// 4. DEFECT V65-D4 (P3) — THE RECEIPT NAMES THE FOUNDER'S ENTITY UNGRAMMATICALLY OR WRONGLY.
...
// ==========================================================================================
{
  const r = receiptOf({ requestedIntent: { verb: 'archive', field: null }, commandText: 'archive the person Bat',
    result: { summary: 'x' } });
  check('DEFECT', 'V65-D4a the receipt says "people", not "persons"',
...
}
{
  const r = receiptOf({ requestedIntent: { verb: 'archive', field: null }, commandText: 'archive the purchase order PO-9',
    result: { summary: 'x' } });
  check('DEFECT', 'V65-D4b a purchase order is not reported as a work order',
...
// ==========================================================================================
{
  ...
  check('CONTRACT', 'V65-C4 the receipt picks an honest reason (kills the two mutants #67 left open)',
    bad === 0, ex.join(' | '));
}
...
// ==========================================================================================
{
  check('CONTRACT', 'V65-C5 index.ts imports nothing from _shared (the deploy surface is index.ts alone)',
  ...
  check('CONTRACT', 'V65-C6 index.ts is CRLF-pure (0 bare LF)', bareLF === 0, 'bare LF = ' + bareLF);
}
