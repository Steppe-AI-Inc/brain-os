// RUN13 DEFECT CASES — verifier #13, campaign #73, base ace9b6a
// (index.ts sha256 021c8989de675035709f48e438d590b2e417677a9625f80a9708cd2dd8a4b786).
//
// Same two-kind convention as v10_/v11_/v12_regression_additions.mjs:
//   CONTRACT — a property that ALREADY HOLDS on ace9b6a but that NO committed suite
//              observes (a SURVIVING mutant in this campaign's mutation battery).
//   DEFECT   — a real escape or regression demonstrated on ace9b6a. Expected = the FIXED
//              behaviour, so these FAIL on ace9b6a BY DESIGN until the fix lands.
//
...
//   * D98 + D99 are the gate blockers and share ONE validated single-hunk fix
//     (qa/verification/proposed/v13_fixes.patch.md, FIX-3b): it takes assertion leaks
//     18/20 -> 0/20 AND dropped clarifications 7/27 -> 0/27 with the whole committed
//     battery still green at 561/561. Do not close D99 by reopening D98 (that is exactly
//     what run11 -> run12 did in both directions).
...
const run = ({ claims = null, summary = '', pendingAction = null, questions, evidence = [], context = {}, grounded = false } = {}) =>
...
console.log(`\nv13_regression_additions: ${pass} pass, ${fail} fail (${defectsOpen} open #73 defects reproduce; ${fail - defectsOpen} CONTRACT failures = guards that do not hold as claimed)`);
