...
const MUTANTS = [
  ...
  { id: 'N13', suite: NT, what: 'a run that cannot renew its lease keeps working', edits: [[NODE, "if (!stopped && monoNow() - leaseFrom > leaseMs - margin) abort(", "if (false) abort("]] },
  { id: 'N13', suite: NT, what: 'the abort guard times the lease from the claim\'s return, not its start', label: 'N13c', edits: [[NODE, '  let lost = false, stopped = false, inFlightSince = 0, leaseFrom = claimedFrom, retry = null;', '  let lost = false, stopped = false, inFlightSince = 0, leaseFrom = monoNow(), retry = null;']] },
  { id: 'N24', suite: NT, what: 'health says "can claim work" over a refused admission or a busy claim lock', edits: [[NODE, '  const claiming = working && !notClaiming;', '  const claiming = working;']] },
  { id: 'S3', suite: ACC, what: 'the oversized-surface exclusion counts characters, not bytes', edits: [[CLAIM, "btrim(s) = '' or octet_length(s) > 1000)", "btrim(s) = '' or length(s) > 1000)"]] },
  ...
  { id: 'N25', suite: NT, what: 'a renewal that lands after the completion is read as a takeover (the heartbeat is never settled)', edits: [[NODE, 'settle: () => { finishing = true; }', 'settle: () => { }']] },
  { id: 'N26', suite: NT, what: 'a completion that meets a transient plane loss is not retried (the run is thrown away)', edits: [[NODE, 'const fin = await retryTransient(() => { completionAttempts++;', 'const fin = await ((f) => f())(() => { completionAttempts++;']] },
  ...
  { id: 'N2', suite: NT, what: 'the worker never re-asserts its role', edits: [[NODE, "n.last_heartbeat_at end, security_role = $2, capabilities", "n.last_heartbeat_at end, security_role = was.security_role, capabilities"]] },
  ...
  { id: 'N6', suite: NT, what: 'one transient plane error ends the worker', edits: [[NODE, '    const run = await retryTransient(async () => {', '    const run = await ((f) => f())(async () => {']] },
  ...
  { id: 'S', suite: ACC, what: 'a repeated, NULL, empty or oversized surface starves the plane', edits: [
    [CLAIM, "            and not exists (select 1 from unnest(wo.owned_surface) s where s is null or btrim(s) = '' or octet_length(s) > 1000)\n", ''],
    [CLAIM, '      wo.owned_surface = [...new Set(wo.owned_surface || [])];\n', '']] },
  { id: 'N16', suite: NT, what: 'a claim-lock record left by an earlier worker is never replaced', edits: [[NODE, '  let busySeen;', '  let busySeen = null;'], [NODE, "for (const f of ['node-admission.json', 'node-claim-busy.json']) {", "for (const f of ['node-admission.json']) {"]] },
  { id: 'N28', suite: NT, what: 'an earlier worker\'s claiming records are repeated for the next worker until its first claim cycle', edits: [[NODE, "for (const f of ['node-admission.json', 'node-claim-busy.json']) { try { rmSync(join(STATE_DIR, f), { force: true }); } catch { /* none */ } }", '']] },
  { id: 'N29', suite: NT, what: 'a claim whose reply was lost leaves its run in progress, unworked, for a whole lease', edits: [[NODE, '      if (claimTries++ > 0 || monoNow() < orphanCheckUntil) await giveBackOrphans(id, mine, log);', '      claimTries++;']] },
  { id: 'N29b', suite: NT, what: 'the orphan check runs only on the retry itself (a COMMIT landing later stands unworked for a lease)', edits: [[NODE, '      if (claimTries++ > 0 || monoNow() < orphanCheckUntil) await giveBackOrphans(id, mine, log);', '      if (claimTries++ > 0) await giveBackOrphans(id, mine, log);']] },
  { id: 'N37b', suite: NT, what: 'two_machine_real reports a run cut short after a FAIL as INCONCLUSIVE', edits: [['qa/factory/two_machine_real.mjs', "const say = (good, label, detail) => { if (!good) { ok = false; judgedFail = true; }", "const say = (good, label, detail) => { if (!good) { ok = false; }"]] },
  { id: 'N38', suite: NT, what: 'the supervisor\'s backoff runs on the wall clock', edits: [['scripts/factory-runner/node-supervisor.mjs', '  const until = performance.now() + backoff;\n  while (performance.now() < until && !stopRequested()) await sleep(Math.min(2000, until - performance.now()));', '  const until = Date.now() + backoff;\n  while (Date.now() < until && !stopRequested()) await sleep(Math.min(2000, until - Date.now()));']] },
  { id: 'N38u', suite: NT, what: 'the supervisor reports its worker\'s uptime from the wall clock', edits: [['scripts/factory-runner/node-supervisor.mjs', 'childUpMs: status && status.childPid ? Math.round(performance.now() - childStartedMono) : null', 'childUpMs: status && status.childPid ? Math.round(Date.now() - Date.parse(status.childStartedAt)) : null']] },
  { id: 'N40', suite: NT, what: 'a run starts at its claim transaction\'s BEGIN (a claim that waited on the lock reads as an overlap)', edits: [[CLAIM, "now() + ($3 || ' seconds')::interval, now(), clock_timestamp(), $2, $4, $5, $6, $7)", "now() + ($3 || ' seconds')::interval, now(), now(), $2, $4, $5, $6, $7)"]] },
  { id: 'N41', suite: NT, what: 'a scheduling wave renews nothing while it goes on claiming', edits: [['qa/factory/two_machine_scheduling.mjs', "        console.log('  claimed ' + wo.title.split(' ').pop() + ' as run ' + run.run_id.slice(0, 8));\n        await beatHeld();", "        console.log('  claimed ' + wo.title.split(' ').pop() + ' as run ' + run.run_id.slice(0, 8));"]] },
  { id: 'N35b', suite: NT, what: 'plane-health ignores --runner-env', label: 'N35p', edits: [['scripts/factory-runner/plane-health.mjs', "  if (reIdx > -1 && process.argv[reIdx + 1]) {\n    const { loadRunnerUrl } = await import('./runner-env.mjs');", "  if (false) {\n    const { loadRunnerUrl } = await import('./runner-env.mjs');"]] },
  { id: 'N35b', suite: NT, what: 'a URL copied from another machine keeps that machine\'s CA path', label: 'N35d', edits: [[DB, "export const FACTORY_RUNNER_PG_URL = process.env.FACTORY_RUNNER_PG_URL ? resolveCaPath(process.env.FACTORY_RUNNER_PG_URL).url : '';", "export const FACTORY_RUNNER_PG_URL = process.env.FACTORY_RUNNER_PG_URL || '';"]] },
  { id: 'N42', suite: NT, what: 'a node that admission refuses keeps stamping liveness (it reads ALIVE while it claims nothing)', edits: [[NODE, '          const b = await nodeBeat(id, nodeRole(), reg, !refusedNow);', '          const b = await nodeBeat(id, nodeRole(), reg, true);']] },
  { id: 'N39', suite: NT, what: 'the idle liveness beat runs on the wall clock', edits: [[NODE, '  let lastBeat = monoNow();', '  let lastBeat = Date.now();'], [NODE, 'ready = true; lastBeat = monoNow();', 'ready = true; lastBeat = Date.now();'], [NODE, '      if (ready && monoNow() - lastBeat >= NODE_BEAT_MS) {', '      if (ready && Date.now() - lastBeat >= NODE_BEAT_MS) {'], [NODE, '          lastBeat = monoNow();', '          lastBeat = Date.now();']] },
  { id: 'N35', suite: NT, what: 'the runbook\'s scripts retry nothing (one reset ends a wave)', label: 'N35r', edits: [['qa/factory/two_machine_scheduling.mjs', '      if (!nodeMod.isTransientPlaneError(e) || Date.now() > until) throw e;', '      throw e;']] },
  { id: 'N35', suite: NT, what: 'the runbook\'s scripts end with exit 1 (the FAIL verdict\'s code) on a plane they cannot reach', label: 'N35x', edits: [['qa/factory/two_machine_scheduling.mjs', "  console.log('INCONCLUSIVE: ' + m + ' - nothing was judged; run the same command again'); process.exit(4);", "  console.log('INCONCLUSIVE: ' + m + ' - nothing was judged; run the same command again'); process.exit(1);"]] },
  { id: 'N32', suite: NT, what: 'a run\'s own statements on the plane are not retried (the handler\'s verification write)', edits: [[NODE, "        retry: (fn, what) => retryTransient(() => { if (hb.signal.aborted) throw hb.signal.reason || new Error('run aborted'); return fn(); }, (what || 'a statement') + ' of run ' + short, log),", '        retry: (fn) => fn(),']] },
  { id: 'N27b', suite: NT, what: 'a node that started its wave first takes both conflict work orders', edits: [['qa/factory/two_machine_scheduling.mjs', "      if (wo.title.split(' ').pop().startsWith('conflict') && held.some((h) => h.kind.startsWith('conflict'))) continue;\n", '']] },
  { id: 'N30', suite: NT, what: 'the work-order read right after a claim is not retried', edits: [[NODE, "      const woRow = (await retryTransient(() => db.read(", "      const woRow = (await ((f) => f())(() => db.read("]] },
  { id: 'N31', suite: NT, what: 'the first renewal is due a third of the lease after the claim returned', edits: [[NODE, 'Math.max(0, claimedFrom + everyMs - monoNow())', 'everyMs']] },
  { id: 'N37', suite: NT, what: 'two_machine_real ends on one transient plane error (no retry)', edits: [['qa/factory/two_machine_real.mjs', 'const db = { ...dbRaw, read: retrying(dbRaw.read), write: retrying(dbRaw.write) };', 'const db = dbRaw;']] },
  { id: 'N37', suite: NT, what: 'two_machine_real reports a plane it cannot reach as exit 1 (read as a FAIL verdict)', label: 'N37x', edits: [['qa/factory/two_machine_real.mjs', "'; run it again'); process.exit(4); };", "'; run it again'); process.exit(1); };"]] },
  { id: 'N20', suite: NT, what: 'the composer counts a failover whose takeover never completed', label: 'N20d', edits: [['qa/factory/factory_v1_acceptance.mjs', " and tr.status = 'done' and tr.termination_reason in ('factory_acceptance_takeover', 'two_machine_failover_completed') and tw.status = 'done'", '']] },
  { id: 'N33', suite: NT, what: 'a renewal that hangs holds every later one back', edits: [[NODE, '    if (inFlightSince && monoNow() - inFlightSince < hungMs) return;', '    if (inFlightSince) return;']] },
  { id: 'N34', suite: NT, what: 'the lease guard runs on the wall clock', edits: [[NODE, 'export const monoNow = () => performance.now();', 'export const monoNow = () => Date.now();']] },
  { id: 'N35', suite: NT, what: 'the runbook\'s scripts read FACTORY_RUNNER_PG_URL raw (no env file, no CA resolution)', edits: [['qa/factory/two_machine_scheduling.mjs', "  if (envArg || !process.env.FACTORY_RUNNER_PG_URL) {", "  if (!process.env.FACTORY_RUNNER_PG_URL) { console.log('FACTORY_RUNNER_PG_URL is not set (the designed refusal)'); process.exit(2); }\n  if (false) {"]] },
  { id: 'N36', suite: NT, what: 'node.mjs status says UNREACHABLE on one failed probe', edits: [[NODE, "    for (let i = 1; i < 3 && s.state === 'UNREACHABLE'; i++) {", "    for (let i = 1; i < 1 && s.state === 'UNREACHABLE'; i++) {"]] },
  { id: 'N27', suite: NT, what: 'the scheduling wave ends at the hold and a minute (a verifier that started first gives up)', edits: [['qa/factory/two_machine_scheduling.mjs', '  const until = Date.now() + (2 * hold + 60) * 1000;', '  const until = Date.now() + (hold + 60) * 1000;']] },
