...
const MUTANTS = [
  ...
  { id: 'N6', suite: NT, what: 'one transient plane error ends the worker', edits: [[NODE, "    const run = await retryTransient(() => { claimStart = Date.now(); return claimWork({ nodeId: id, leaseSeconds, requestedProvider, requestedModel, workTypes, baseCommit: commit }); }, 'claim', log);", '    const run = await claimWork({ nodeId: id, leaseSeconds, requestedProvider, requestedModel, workTypes, baseCommit: commit });']] },
