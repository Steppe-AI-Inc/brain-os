...
const MUTANTS = [
  ...
  { id: 'N13', suite: NT, what: 'the abort guard times the lease from the claim\'s return, not its start', label: 'N13c', edits: [[NODE, '  let lost = false, stopped = false, inFlight = false, leaseFrom = claimedFrom, retry = null;', '  let lost = false, stopped = false, inFlight = false, leaseFrom = Date.now(), retry = null;']] },
  ...
  { id: 'N31', suite: NT, what: 'the first renewal is due a third of the lease after the claim returned', edits: [[NODE, 'Math.max(0, claimedFrom + everyMs - Date.now())', 'everyMs']] },
