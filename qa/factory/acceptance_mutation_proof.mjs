...
const MUTANTS = [
  ...
  { id: 'N39', suite: NT, what: 'the idle liveness beat runs on the wall clock', edits: [[NODE, '      if (ready && monoNow() - lastBeat >= NODE_BEAT_MS) {', '      if (ready && Date.now() - lastBeat >= NODE_BEAT_MS) {']] },
  { id: 'N35', suite: NT, what: 'the runbook\'s scripts retry nothing (one reset ends a wave)', label: 'N35r', edits: [['qa/factory/two_machine_scheduling.mjs', 'const db = { ...dbRaw, read: retrying(dbRaw.read), write: retrying(dbRaw.write) };', 'const db = dbRaw;']] },
