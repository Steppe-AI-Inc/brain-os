...
const TITLE = 'TMR-';
// THE COMMIT UNDER ACCEPTANCE: --sha, or this checkout's HEAD. Both nodes must run exactly it, clean, with the acceptance handler
// this seeder speaks - nothing checked that, and a Work PC on an older checkout passed this acceptance and completed an action
// its handler did not know (final verification 2026-09-24). Each node records its commit and handler as capabilities.
const { execFileSync } = await import('node:child_process');
const EXPECTED = opt('--sha') || execFileSync('git', ['rev-parse', 'HEAD'], { cwd: ROOT, encoding: 'utf8' }).trim();
const HANDLER_CAP = 'handler:factory-acceptance/2';
const headOf = (caps) => { const h = (caps || []).find((c) => String(c).startsWith('head:')); return h ? String(h).slice(5) : null; };
const hostOf = (platform) => String(platform || '').split(' ')[1] || '';

const aliveNodes = async () => (await db.read("select node_id, security_role, platform, capabilities, extract(epoch from (now() - last_heartbeat_at)) as age from factory.nodes where last_heartbeat_at > now() - interval '3 minutes' order by last_heartbeat_at desc")).rows
  .map((n) => ({ nodeId: n.node_id, role: n.security_role, host: hostOf(n.platform), ageS: Math.round(Number(n.age)), head: headOf(n.capabilities), dirty: (n.capabilities || []).includes('dirty'), handler: (n.capabilities || []).includes(HANDLER_CAP) }));

if (mode === 'nodes') {
  const nodes = await aliveNodes();
  if (!nodes.length) console.log('no ALIVE node on the plane (heartbeat within 3 min)');
  for (const n of nodes) console.log(n.nodeId.slice(0, 20) + '  ' + n.role.padEnd(14) + ' ' + n.host + '  heartbeat ' + n.ageS + ' s ago  ' + (n.head ? n.head.slice(0, 12) + (n.dirty ? '+dirty' : '') : 'commit unknown') + (n.head === EXPECTED && !n.dirty ? ' (= this checkout)' : ''));
  process.exit(0);
}
if (mode === 'cleanup') {
...
if (mode !== 'run') { console.log('usage: two_machine_real.mjs nodes | run [--home <nodeId>] [--work <nodeId>] [--sha <commit>] | cleanup'); process.exit(2); }

const nodes = await aliveNodes();
const home = nodes.find((n) => n.nodeId === (opt('--home') || nodeMod.nodeId()));
const verifiers = nodes.filter((n) => n.role === 'verifier' && n.nodeId !== (home && home.nodeId));
const work = opt('--work') ? nodes.find((n) => n.nodeId === opt('--work')) : (verifiers.find((n) => n.head === EXPECTED && !n.dirty && n.handler) || verifiers[0]);
...
console.log('acceptance of commit ' + EXPECTED);
...
// S3 scheduling
{
  ...
  // done runs alone called two overlapping executions on one surface "never at the same time" (final verification 2026-09-24)
  const win = (r) => [new Date(r.started_at).getTime(), new Date(r.finished_at || r.last_heartbeat_at || r.started_at).getTime()];
  const disjoint = ra && rb && runsA.every((x) => runsB.every((y) => { const [s1, e1] = win(x), [s2, e2] = win(y); return e1 <= s2 || e2 <= s1; }));
...
if (home.host && work.host && home.host !== work.host) { console.log('VERDICT: TWO MACHINES at ' + EXPECTED + ' — ' + home.host + ' and ' + work.host + ': failover both ways, conflict-aware scheduling and verifier independence held on real machines, both running exactly this commit (stamp ' + stamp + ').'); process.exit(0); }
