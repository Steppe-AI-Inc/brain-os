...
try {
  ...
  if (FRESH) {
    ...
    { const d = clone('fsupcommit'); edit(d, 'scripts/factory-runner/install-autostart.ps1', (s) => s.replace('function Test-SupervisorCommit($sv) { if (-not $sv -or -not $CheckoutHead) { return $true }; if (-not $sv.head) { return $false }; return (($sv.head -eq $CheckoutHead) -and ([bool]$sv.dirty -eq $CheckoutDirty)) }', 'function Test-SupervisorCommit($sv) { return $true }')); commitAll(d, 'mutant: the supervisor\'s commit is not compared');
      expectRed('F-SUPCOMMIT', 'a supervisor still on the old commit, whose worker restarted on the new one, passes -Verify and -Start leaves it', d, ['F6'], ['--skip', 'F7,F8,F9,F10,F11,F12,F13,F14,F15']); }
    { const d = clone('funready'); edit(d, 'scripts/factory-runner/install-autostart.ps1', (s) => s.replace('elseif (-not $live.legacy -and -not $live.readyAt) {', 'elseif ($false) {').replace('elseif (-not $sv.legacy -and -not $sv.readyAt) {', 'elseif ($false) {')); commitAll(d, 'mutant: a worker that has not completed a claim cycle reads ALIVE');
      expectRed('F-UNREADY', '-Status and -Verify read ALIVE for a worker that has not completed a claim cycle (its predecessor\'s heartbeat)', d, ['F6'], ['--skip', 'F7,F8,F9,F10,F11,F12,F13,F14,F15']); }
    { const d = clone('fleavealone'); edit(d, 'scripts/factory-runner/install-autostart.ps1', (s) => s.replace("if ((-not $plane -or $plane.state -eq 'UNREACHABLE') -and $sv.readyAt -and (Test-SupervisorCommit $sv)) {", 'if ($false) {')); commitAll(d, 'mutant: -Start restarts a ready node when the plane does not answer its probe');
      expectRed('F-LEAVEALONE', '-Start restarts a healthy node whose plane did not answer this PC\'s status probe', d, ['F6'], ['--skip', 'F7,F8,F9,F10,F11,F12,F13,F14,F15']); }
    { const d = clone('fadmlog'); edit(d, 'scripts/factory-runner/install-autostart.ps1', (s) => s.replace('  $mine = if ($from -ge 0) { $tail[$from..($tail.Count - 1)] } else { @() }', '  $mine = $tail')); commitAll(d, 'mutant: any worker\'s admission refusal in the log is the current one\'s');
      expectRed('F-ADMISSIONLOG', '-Start quotes an earlier worker\'s admission refusal for a worker that cannot reach the plane', d, ['F6'], ['--skip', 'F7,F8,F9,F10,F11,F12,F13,F14,F15']); }
    { const d = clone('fverifyother'); edit(d, 'scripts/factory-runner/install-autostart.ps1', (s) => s.replace("  if (Test-OtherCheckout $owner) { \"task      $TaskName\"; \"workdir   $owner\"; \"FAIL the task '$TaskName' belongs to another checkout ($owner) - run -Verify there\"; exit 1 }", '')); commitAll(d, 'mutant: -Verify reads another checkout\'s env file and plane');
      expectRed('F-VERIFYOTHER', '-Verify on another checkout\'s task reads its env file (and would probe its plane)', d, ['F6'], ['--skip', 'F7,F8,F9,F10,F11,F12,F13,F14,F15']); }
    { const d = clone('fguardfirst'); edit(d, 'scripts/factory-runner/install-autostart.ps1', (s) => s.split('\r\n').join('\n').replace("# and -Start read and judged that checkout's credential file, and refused for the wrong reason when it was unusable (final verification 5)\nif ((Test-OtherCheckout $owner) -and -not $ReplaceOtherCheckout) { Deny-OtherCheckout 'changed' }\n", "# (mutant)\n")); commitAll(d, 'mutant: another checkout\'s task is refused only after its env file was read and judged');
      expectRed('F-GUARDFIRST', '-Start on another checkout\'s task reads and judges that checkout\'s env file before refusing', d, ['F6'], ['--skip', 'F7,F8,F9,F10,F11,F12,F13,F14,F15']); }
    { const d = clone('flasterror2'); edit(d, 'scripts/factory-runner/install-autostart.ps1', (s) => s.replace('  if ($marks.Count -ge 2) { $segs += ,@($tail[$marks[-2]..($marks[-1] - 1)]) }', '')); commitAll(d, 'mutant: the backoff\'s error is looked for only after the newest worker\'s start');
      expectRed('F-LASTERROR2', 'in backoff, a worker started a moment ago hides the error of the one that exited', d, ['F6'], ['--skip', 'F7,F8,F9,F10,F11,F12,F13,F14,F15']); }
    { const d = clone('fnotstarted'); edit(d, 'scripts/factory-runner/install-autostart.ps1', (s) => s.replace("$_ -match '^(error: |REFUSED|NOT STARTED)' -or $_ -match '\\] (error: |REFUSED|NOT STARTED)'", "$_ -match '^(error: |REFUSED)' -or $_ -match '\\] (error: |REFUSED)'")); commitAll(d, 'mutant: a worker refused as a second worker is "nothing logged"');
      expectRed('F-NOTSTARTED', 'beside a bare worker, -Start says "nothing logged that names it"', d, ['F6'], ['--skip', 'F7,F8,F9,F10,F11,F12,F13,F14,F15']); }
    { const d = clone('fpower'); edit(d, 'scripts/factory-runner/install-autostart.ps1', (s) => s.split('\r\n').join('\n').replace("  $lines | Where-Object { $_ -is [string] }\n  Get-SleepNote\n", "  $lines | Where-Object { $_ -is [string] }\n")); commitAll(d, 'mutant: the preflight says nothing about the power plan');
      expectRed('F-POWER', 'the preflight does not say the sleep timeouts', d, ['F6'], ['--skip', 'F7,F8,F9,F10,F11,F12,F13,F14,F15']); }
