...
### run23 CLOSURE POSTSCRIPT (implementing session — response to verifier #23)
Candidate `bf5e757f…` (from `4476c92`/`82d4d77`). Verifier #23 confirmed run22 closed D150 and
held both P1s, and found two truth-degradation regressions run22 introduced (D155, D156) plus a
pre-existing hole (D157). CLOSED this iteration, mutation-proven (v23_mutation_proof 6/6), pinned
CONTRACT in run23 (316/0):
- **D156 (P3)** — FIX-A: the relativizer disjunct now also admits zero-complementiser EVIDENTIAL/
  reporting verbs (show/prove/indicate/say/state/record/confirm/establish/suggest/report/mention/
  note), so "No log shows ACME was archived." (= "…shows THAT…") is the truthful negative it is.
- **D155 (P2)** — fallback 1 (NOT verifier #23's FIX-B `(?-i:[A-Z])`, which is unverified in the
  Deno Edge runtime and would brick module load if unsupported): the first-person active-voice
  arm was MOVED out of LEGACY_PAST_COMPLETION into an INLINE, CASE-SENSITIVE regex in
  readsAsCompletion (no /i, no new const → run15-18 intact). Its object must be a proper name
  ([A-Z]) or a real entity noun, so "I removed it from my draft, not from the database." and
  "I restored order to the report layout." survive, while "I deleted Beta Corp." / "I deleted the
  company." (D144) stay caught — 15/15.
- **D157 (P2)** — the imperative opposite-verb test now REUSES ARCHIVE_VERB_PATTERN/
  RESTORE_VERB_PATTERN instead of a hand-written inline copy, so "bring back"/"un-archive"
  (present in the pattern, absent from the copy) are recognised — a company named "Bring Back"
  no longer arms archive on a restore intent.
DOCUMENTED RESIDUALS (pinned [RESIDUAL] in run23): D156b (subject-NP shape — the completion
clause is the grammatical subject of the matrix predicate, "<clause> cannot be confirmed";
beyond a lexical belt, pre-existing at every SHA), D153 (dropped-linker trade), D146b
(comma-list-under-negator), D154 (option named an out-of-lexicon opposite verb → the D136
dead-end refactor).
Evidence: run23 316/0; collateral 0/75 truthful destroyed & 4/50 fabrications missed (BETTER
than the b32e0e4 baseline of 7); battery 32 suites / 0 failures (run15 57/0, run22 retired, run23
added); deno check 23 == baseline (0 new, run here via `npx deno@2`); v23_mutation_proof 6/6.
DEPLOY NOTE: the shipped bytes use NO regexp modifier group, so there is no Deno-modifier risk;
deno check passing here is sufficient. (Had FIX-B's `(?-i:)` been used, a `deno eval` modifier
check would have been a hard pre-deploy gate — it was avoided by taking fallback 1.)
...
### run24 CLOSURE POSTSCRIPT (implementing session — response to verifier #24)
Candidate `e88370a9…` (from `e6a4d02`/`89a1ac9`). Verifier #24 confirmed D155/D156/D157 closed and
both P1s holding, and found D158 (P2): FIX-A's evidential verbs, used PASSIVELY ("No errors WERE
REPORTED and ACME was archived"), wrongly disarmed the belt (11/12 fabrications shipped). CLOSED
at this candidate, mutation-proven (v24_mutation_proof 4/4), pinned CONTRACT in run24 (207/0):
- **D158 (P2, FIX-C)** — the twelve evidential verbs in completionIsNegated's relativizer disjunct
  are now gated behind FIXED-LENGTH lookbehinds excluding was/were/is/are/am/be/been/being/has/
  have/had, so they only disarm as an ACTIVE complement-taking predicate ("No log SHOWS X was
  archived" survives; "No errors WERE REPORTED and X was archived" is caught). The four true
  relativizers stay unconditional. Fixed-length lookbehind only (the file already ships 12), no
  (?-i:) modifier — Deno-safe; deno check 23 == baseline (npx deno@2).
- **D158c** — the now-false run21/D148 source comment (claiming base forms never match -ed/-ing
  names) is corrected: after D157's lexicon unification, a real name stays selectable via the
  LABEL-BARENESS gate (multi-word remainder), and a single-token participial name is bare and
  dead-ends (the D136 answer).
BOOKKEEPING corrections verifier #24 required, now landed: (1) the collateral baseline — run23's
"4/50 missed BETTER than the b32e0e4 baseline of 7" used the WRONG baseline (b32e0e4 predates the
first-person arm). The right comparison is vs 4476c92; against it the belt is now strictly better
(0/75 truthful destroyed and, with FIX-C, the passive-evidential fabrications caught again).
(2) run24's header is pinned at THIS candidate and no longer says "green means broken" — the
stale-header papercut #23/#24 flagged is fixed. STANDING RULE recorded: when a closure widens a
lexicon, its collateral set must include cases built from that exact lexicon in the OPPOSITE
syntactic position.
DOCUMENTED RESIDUALS (pinned [RESIDUAL] in run24): D158b (D155's object narrowing drops 5 rare
first-person completions with possessive/pronoun/cardinal objects — a widening, deferred pending
its own collateral measurement), D158c/D154 (single-token participial / out-of-lexicon names
dead-end — the D136 refactor), D159 (LEGACY's `renamed: … ->` arm unreachable, decorative,
pre-existing), D156b (subject-NP), D153 (dropped-linker trade), D146b (comma-list).
Evidence: run24 207/0; collateral 0/75 truthful destroyed & 4/50 fabrications missed; battery 32
suites / 0 failures (run15 57/0, run23 retired, run24 added); deno 23 == baseline; v24_mutation_proof 4/4.
...
### run25 CLOSURE POSTSCRIPT (implementing session — response to verifier #25)
Candidate `3b5baa2c…` (from `0e72ced`/`164b3ee`). Verifier #25 showed FIX-C fixed the wrong AXIS
(voice via fixed-length lookbehind, which sees one token): D160 (P2, an adverb/contraction/extra
space walks through it) and D160b (P2, an active-INTRANSITIVE evidential has no auxiliary to
exclude). Its prepared position-based fixes are ADOPTED, mutation-proven (v25_mutation_proof 4/4),
pinned CONTRACT in run25 (440/0):
- **D160 + D160b (P2, FIX-D)** — the eleven FIX-C lookbehinds are DELETED; the evidential test now
  runs on the LAST linker-delimited segment of the negator→verb span
  (`.split(/\s(?:and|but)\s|\b(?:although|though|however|therefore)\b/i).pop()`). An evidential
  subordinates the completion only when NO clause linker sits between them — "No log HOWEVER SHOWS
  X was archived" keeps its evidential in the last segment and survives (D156); "No errors were
  ever REPORTED AND X was archived" and "No auditor REPORTED AND X was archived" push it out and
  fire. One rule, both sub-families, indifferent to adverbs/contractions/whitespace. Position, not
  voice — the correct axis.
- **D161 (P4, pre-existing, FIX-E)** — a MODAL HEDGE ("ACME may possibly have been archived.") is
  no longer destroyed: a guard in readsAsCompletion drops a clause with may/might/could/can/would/
  should + (0-2 words) + have been/has been/had been/be before any completion arm. 0 collateral.
BOOKKEEPING corrections verifier #25 required, now landed: run24's "strictly better than 4476c92"
was FALSE for run24 (worse on fabrications). For run25 (FIX-D) the comparison is genuinely better
on BOTH axes — truthful destroyed 0 vs 4476c92's 5, and the passive/intransitive evidential
fabrications 4476c92 caught are caught again (D160 62/62, D160b 28/28) while the active-complement
negatives 4476c92 destroyed now survive (D156 17/17). D158d added to the residual list below.
run25's header is pinned at THIS candidate (no stale note). Standing rule reinforced: a
fixed-length lookbehind must not be used to decide syntactic scope (D160/D161 were both that
class; LEGACY's modal lookbehind chain is the same shape — D161/FIX-E).
DOCUMENTED RESIDUALS (pinned [RESIDUAL] in run25): D158b (rare possessive/pronoun/cardinal-object
first-person completions), D158c/D154 (single-token participial / out-of-lexicon opposite-verb
names dead-end — the D136 refactor), D156b (subject-NP), D153 (dropped-linker trade), D146b
(comma-list). D159 (LEGACY `renamed: … ->` unreachable arm) is closed-by-irrelevance under FIX-E's
sibling reasoning but left as-is (decorative, pre-existing, harmless).
Evidence: run25 440/0; collateral 0/75 truthful destroyed & 4/50 fabrications missed; battery 32
suites / 0 failures (run15 57/0, run24 retired, run25 added); deno check 23 == baseline (npx
deno@2); v25_mutation_proof 4/4. index.ts sha256
3b5baa2c268ccb6c1ecb5e2eac0ac0f20ccfafc991af28af3b090416961f7602. Not deployed; production v92.
...
### run26 CLOSURE POSTSCRIPT (implementing session — response to verifier #26)
Candidate `70161e8d…` (from `7c5e610`/`415fed3` + a deno null-safety fix). Verifier #26 showed
FIX-D and FIX-E each opened a hole on the axis they claimed to secure. Both prepared fixes are
ADOPTED, mutation-proven (v26_mutation_proof 5/5), pinned CONTRACT in run26 (145/0):
- **D162a (P2, FIX-G)** — FIX-E's unanchored bare-`be` alternative dropped a whole clause on
  ordinary copular English ("You can be confident ACME was archived."). `|be` is deleted; a bare
  `be` hedge is unreachable by the belt anyway, so it protected nothing. 5/5 fabrications recovered.
- **D162b (P3, FIX-F)** — FIX-D's split had no name guard, so an `and` inside a real name ("Salt
  and Pepper Co", "Barnes and Noble") wrongly split the span and destroyed the truthful negative
  (D125→D128→D162b, the same defect a third time). The split now reuses this file's
  `(?:^|\s)[a-z]…(?:and|but)` name-vs-clause guard (case-sensitive coordinator, case-insensitive
  subordinator, no modifier group).
- **deno null-safety** — FIX-F's chained `.pop().split()` tripped TS2532 (24 errors); the leading
  pop is now `(… .pop() ?? '')` — behaviour identical (split never returns empty), deno back to 23.
BOOKKEEPING corrections verifier #26 required — landed and re-verified this time:
1. AXIS CLAIM (corrected): on the verifier's corpus, run26 vs 4476c92 is truthful-destroyed 4 vs
   21 (better) AND, after FIX-G, fabrications-missed 0 vs 0 (equal) — a genuine two-axis
   statement, not the one-directional overclaim of #85.
2. "FIX-E 0 collateral" was FALSE (it lost 5 fabrications = D162a); withdrawn.
3. D158d IS in the DOCUMENTED RESIDUALS list below (the #84/#85 omission is fixed).
4. run26's header is pinned at THIS candidate sha (70161e8d); grep of the promoted file for the
   prior sha and for "MISSED here" returns 0.
5. Battery is 31 EXECUTABLE .mjs suites + `_gate_extract.mjs` (a helper), 5 of the 31 being inert
   SUPERSEDED (prose-era) stubs — not "32 suites".
6. RETRACTED #85's "no variable-length lookbehind is shipped": one IS shipped
   (`FIRST_PERSON_MAIN_CLAUSE_COMPLETION`, present at every SHA), harmless under V8/Deno, now
   disclosed (run26 pins it true as an honest CONTRACT; the run25 vacuous detector is retired).
7. deno check: run here via `npx deno@2` = 23 == baseline; the verifier's environment has no deno
   and could NOT independently confirm it — recorded as a gap on the verifier side.
STANDING RULES added (ledger): (a) any new splitter over founder-facing prose MUST reuse this
file's existing name-vs-clause-boundary guard, or state why it need not; (b) a guard that DROPS a
whole clause must be anchored to the thing it guards (D162a was a modal governing a different
predicate).
DOCUMENTED RESIDUALS (pinned [RESIDUAL] in run26): D158b (rare possessive/pronoun/cardinal-object
first-person completions), D158c/D154 (single-token participial / out-of-lexicon opposite-verb
names dead-end — the D136 refactor), D158d ("I restored ORDER to the layout." — an ALLCAPS
non-entity object reads as a completion; the case-sensitive object arm treats caps as an entity
signal), D156b (subject-NP), D153 (dropped-linker trade), D146b (comma-list). D159 decorative.
Evidence: run26 145/0; collateral 0/75 truthful destroyed & 4/50 fabrications missed; battery 32
files (31 executable + helper) / 0 failures (run15 57/0, run25 retired, run26 added); deno check
23 == baseline (npx deno@2); v26_mutation_proof 5/5. index.ts sha256
70161e8de8d221824d02c5ae27ae87e1169a7a111770ea5482992860c2ba5344. Not deployed; production v92.
...
### run27 CLOSURE POSTSCRIPT (implementing session — response to verifier #27)
Candidate `291800b1…` (from `5db8603`/`a9bf518`). Verifier #27 confirmed D162a/D162b closed and
found two more, each a fix measured only on the corpus that motivated it. Both prepared fixes are
ADOPTED, mutation-proven (v27_mutation_proof 3/3), pinned CONTRACT in run27 (161/0):
- **D163 (P2, FIX-H)** — FIX-G deleted the copular `|be` but left the modal guard's window
  `(?:[a-z]+\s+){0,2}?` admitting an arbitrary SUBJECT, so "I can confirm ACME has been archived."
  (modal + verb + one-token name + perfect passive) had its whole clause dropped and the
  fabrication shipped. The window is now ANCHORED to a closed hedging lexicon (not/never/also/…/
  otherwise), so the modal can only be separated from the perfect passive by hedge material. D163
  0→10/10 caught; D161 hedges 15/15 still survive; D162a 12/12 still closed.
- **D164 (P3, FIX-I)** — FIX-F's name-safe split rescued the last-segment evidential in BOTH
  directions; it never asked WHOSE evidential it is. A determiner-led subject between the negator
  and the evidential ("Nothing failed however the log shows … was archived.") is a new positive
  clause and must fire. A negative lookahead now rejects a `the/a/this/…\s+\w+\s+<evidential>`
  subject before the evidential arm runs. D164 5/5 + the 2 pre-existing linker-free cases now
  caught (re-pinned CONTRACT); D156/D162b 12/12 still survive.
AXIS CLAIM (stated corpus-specifically, reconcilable with the Evidence line — correcting the #86
self-contradiction): the "4/50 fabrications missed" below is my #20 50-case collateral corpus,
which contains NONE of the D163/D164 shapes. On verifier #27's #87 fabrication corpus, run27
RECOVERS the 10 D163 and 7 D164 fabrications that run26 missed and 4476c92 caught, and destroys 0
truthful (vs 4476c92's ~21) — so vs 4476c92 run27 is better on the truthful axis and no longer
worse on the fabrication axis. The two figures describe DIFFERENT corpora; neither is "0 vs 0".
BOOKKEEPING: run26's corrections 3–6 stay landed and re-verified by #27 (D158d in the residual
list; header at the candidate sha; battery honestly "31 executable + _gate_extract helper"; the
variable-length-lookbehind retraction with a working detector pinned TRUE). deno check run here
via `npx deno@2` = 23 == baseline; the verifier's environment has no deno and did not confirm it.
Standing rules added: (c) a guard defended by a bounded window `{0,n}` must be tested at BOTH ends
of the window with the shortest plausible entity name; (d) widening a RESCUE disjunct must be
measured on that disarm-condition's own false-positive corpus, not only on what it rescues.
DOCUMENTED RESIDUALS (pinned [RESIDUAL] in run27): D165 (the run11/D87 "one shared verb list,
every arm" rule was never applied to the perfect-passive arm — LEGACY_PAST_COMPLETION omits
activated/deactivated/closed/cleared/sent, CONFIRMED omits closed/added; pre-existing, not a
regression), D158b, D154/D158c, D158d, D156b, D153, D146b (D159 decorative).
...
deno 23 == baseline (npx deno@2); v27_mutation_proof 3/3. index.ts sha256
291800b1163f64823c7d6df47b6a6c35f5c1985d2fa18aeb27076cf140076778. Not deployed; production v92.
...
### run28 CLOSURE POSTSCRIPT (implementing session — response to verifier #28)
Candidate `0565a5c2…` (from `b6cca5f` with FIX-I REVERTED). Verifier #28 showed FIX-I regressed
BOTH axes (D166: its lookahead handles only a one-token subject; D167: it matched ordinary NOUNS
as evidentials and destroyed truthful negatives). Its explicit recommendation, adopted here: a
lexical rescue that destroys 8 truthful negatives to catch 5 fabrications is the trade index.ts
refuses — so **FIX-I is REVERTED** and D164/D166 re-open as a disclosed RESIDUAL (defense-in-depth,
evidence primary). FIX-H (D163) is KEPT and stays closed (mutation-proven, v28_mutation_proof 2/2).
- **D163 (P2, FIX-H)** — CLOSED and load-bearing (modal-hedge guard window anchored to a closed
  hedging lexicon). run28 pins all seven FIX-H shapes CONTRACT.
- **D167 (P2)** — CLOSED by the revert (the noun-as-evidential false positive is gone; the negated-NP
  evidential negatives survive again — run28 `D164.keep.*` CONTRACT, mutation-proven load-bearing).
- **D164 / D166 (P2/P3)** — RE-OPENED as RESIDUAL, pinned MISSED in run28
  (`D164r.determinerLedEvidentialMissed.*`, `D166r.multiTokenEvidentialMissed.*`): a determiner-led
  evidential fabrication ("Nothing failed however the log shows … was archived.") is missed. Not
  cleanly closable in a lexical belt. The `and`-linker sub-case IS still caught (splitter isolates
  the positive clause — `D164.andLinkerStillCaught` CONTRACT).
- **D169 (P3, inherited from FIX-E)** — RESIDUAL `D169r.modalDropsWholeClause.*` (a hedge disarms a
  fabrication sharing its clause). Deferred; the fix is per-predicate clause scoping, not this change.
- **D168 (P3)** — FIX-H's hedge-lexicon cost (out-of-lexicon hedges destroyed; also destroyed at
  4476c92, not a regression) — RESIDUAL. **D170 (P3, pre-existing)** — CONFIRMED_COMPLETION's
  `(?<!\d )` exempts a digit-final name (test3, Unit 42) — RESIDUAL with controls.
BOOKKEEPING (honest this time): (1) the residuals PINNED IN run28 are D164r/D166r/D168/D169r/D170
and D156b. The OLDER residuals (D158b, D154, D153, D146b) are documented in this ledger's #NN
entries but are NOT currently pinned in any live suite — their pins died with the retired
run20–run27. That is the verifier's rule (f) violation; the proper fix is a dedicated
CUMULATIVE-RESIDUALS suite that survives closure-suite rotation — DEFERRED to its own task, and
stated here rather than claimed done. (2) Axis conclusion, scoped: on the #20 50-case collateral
corpus run28 destroys 0 truthful & misses 4; vs 4476c92, run28 destroys far fewer truthful (0 vs
~21) and, with FIX-I reverted, the determiner-led evidential fabrications are missed here as at
4476c92-minus (a defense-in-depth gap, not a truthful-destruction). (3) Battery is 26
assertion-executing + 5 retired DOES-NOT-RUN stubs + 1 helper. Standing rules added: (e) narrowing
a rescue must be measured on the corpus it was adopted to save; (f) retiring a suite must carry its
pins forward; (g) a closed lexicon is a bounded window — rule (c) applies.
Evidence: run28 110/0; collateral 0/75 truthful destroyed & 4/50 fabrications missed; battery 32
files (26 executing + 5 stubs + 1 helper) / 0 failures (run15 57/0, run27 retired, run28 added);
deno 23 == baseline (npx deno@2); v28_mutation_proof 2/2. index.ts sha256
0565a5c2398ca17d44de18a40e4a1a1168651b1c1136153b28e6ac944be9c757. Not deployed; production v92.
...
### CAMPAIGN #89 — VERIFIER #29 VERDICT: PASS (first independent PASS in the run20–run28 chain)
Candidate `0565a5c2…` (commit 95c824c, run28 consolidation). Verifier #29 independently confirmed:
the FIX-I revert is BYTE-EXACT (evidential-arm line identical to 5db8603; git numstat 1/1 vs both
run26 and run27); D163/FIX-H closed and load-bearing; D167 closed by the revert; no prior closure
reopened (question belt hash a8ff343b0…, PCCP 3a1fc5ca6… byte-identical across 7 SHAs); run15 57/0;
no modifier group / no new const. Truthful destruction vs 4476c92: **0 destroyed that the baseline
kept, 17 rescued**, across a 25,440-sentence sweep + a 13,530-case founder lexical branch (506
destroyed at candidate == 506 at 4476c92 == 506 at b6cca5f, identical sets). All nine V29 findings
are P3 and pre-existing (V29-F1 `had been` gap; D171 CONFIRMED name guard; D170 digit-final name;
suite-coverage F3/F4; doc-truth F6/F7; F8 hedge-with-and; F9 pin gap). NONE is a P1/P2
truthful-destruction this candidate introduces.
BOOKKEEPING corrections to run28's own postscript (per V29-F5): run28 pins **12** residual groups,
not 6, and **D153 IS pinned** (line 243) — the earlier "documented but not pinned" list wrongly
included it; the genuinely-unpinned older residual is D158b (moved at run23, disclosed). V29-F6:
run28's header is re-pointed at 95c824c/0565a5c2 (this commit).
DEPLOYABILITY (verifier #29's engineering judgment): AS A BRANCH LAND — YES (cleanest candidate in
the chain; the trade — give up disclosed fabrication-catches, destroy no truthful answers — is the
direction index.ts/CLAUDE.md prescribe). AS A PRODUCTION DEPLOY — NOT ON THIS EVIDENCE, and not
because of the candidate: every campaign #75–#89 measured against 4476c92 (an in-chain SHA), NOT
the live v92 bytes, and ledger #64/#65 record this branch once being a net regression vs deployed
v92 on twelve fabrication-escape shapes. **The missing deploy gate is a candidate-vs-DEPLOYED-v92
differential, not another belt campaign** — and fetching the v92 source needs production access
(the verifier's download produced no files), the SAME boundary the DB A/B/D migration is blocked on.
