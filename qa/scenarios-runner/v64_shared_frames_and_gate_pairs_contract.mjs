#!/usr/bin/env node
// PROMOTED from verifier #64 (campaign #124), whose FAIL on 15480e3 this candidate closes.
// Two checks were rewritten from modelling an implementation shape to asserting the property: the frame
// tiers are now built from ONE definition (so there is no second regex literal to compare), and the
// namedTargets envelope lives beside its rows rather than inside context.collections.
//
...
const REPO = resolve(HERE, '../..');               // qa/scenarios-runner -> repo root
...
const D1_ROWS = [
  ...
  // Added on closure (2026-09-08). The mixed-script loan verb was in verifier #64's report table but not in
  // its corpus, so the guard that handles it — MN_LOAN_VERB — had no test and the vacuity sweep flagged it.
  // Mongolian carries a borrowed English verb with the light verb хийх: 'archive хийнэ үү' is an ordinary
  // polite request, and it must reach the receipt like any other.
  ['ACME-г archive хийнэ үү', 'mixed-script loan verb: an English verb carried by the Mongolian light verb хийх'],
  ['Тайланг export хийнэ үү', 'same, for a verb the product cannot execute — the receipt must say so truthfully'],
  ['ACME-г archive хий', 'the bare imperative form of the same construction'],
  // POLITE_REQUEST was deleted on closure as a third hand-maintained frame list; these are the cases it used
  // to carry, and they must still work from the shared frame definition alone.
  ['shall i archive ACME?', 'was in POLITE_REQUEST and NOT in the frame list — neither tier had both'],
  ['would you mind archiving ACME?', 'carried by POLITE_REQUEST before it was deleted'],
  ['can we archive ACME?', 'carried by POLITE_REQUEST before it was deleted'],
];
