// =====================================================================================
// run22 CLOSURE CONTRACT (implementing session), promoted from VERIFIER #22 (campaign #82) — regression additions for the run21 closure
...
for (const t of ['There were no blockers so ACME was archived.',
  ...
  belt('RESIDUAL', 'D153.residual.droppedLinkerTrade.' + JSON.stringify(t.slice(0, 44)), t, false,
    'DELIBERATE, DISCLOSED trade: D146 dropped so/because/since/while/before/after/yet/or as linkers to ' +
    'stop destroying truthful negatives; the cost is these 8 aux-preceded fabrications go uncaught. Net ' +
    'truthful destroyed ~1/84 here vs ~37/84 at b32e0e4 — a large net gain, not a silent regression');
}
...
belt('RESIDUAL', 'D152.residual.restoredOrderIdiomFP', 'I restored order to the report layout.', true,
  'the ONE accepted false positive of the D152 clause-anchored active-voice arm: "restored order" is an ' +
  'idiom, not an entity mutation. First-person only — "They restored order after the outage" is safe. ' +
  'Documented, not silently shipped; the arm otherwise catches 7/7 D144 fabrications with 0/12 D145 truthful destroyed');
