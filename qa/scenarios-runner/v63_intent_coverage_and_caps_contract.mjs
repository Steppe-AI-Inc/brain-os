...
// qa/verification/scratch/p1/vacuity_sweep2.mjs, which stubs every named helper and moves every numeric cap.
{
  ...
  const eoa = new AsyncFn('supabase', 'action', 'Deno',
    // executeOneAction calls rpcPostcondition, declared just above it — bring the REAL one along.
    // stripTS cannot detype a GENERIC in parameter position (Record<string, unknown> becomes 'r, unknown>'),
    // a documented limitation of this repo's detyper. Repair the signature here; the durable fix is a
    // module-level type alias in index.ts, which is a source change and belongs to the next round.
    stripTS(src.slice(src.indexOf('function rpcPostcondition('), src.indexOf('\n}', src.indexOf('function rpcPostcondition(')) + 2))
      .replace('function rpcPostcondition(r, unknown>)', 'function rpcPostcondition(r)') + '\n' +
    // stripTS turns ': Promise<{...}>' into a stray '{...}> ' before the body, so consume that too.
    stripTS(src.slice(eoaStart, eoaEnd))
      .replace(/^async function executeOneAction\([^)]*\)(?:[^{]*\{[^}]*\}>\s*)?\s*\{/, 'async function __eoa(supabase, action) {')
