// =====================================================================================
// run23 CLOSURE CONTRACT (implementing session), promoted from VERIFIER #23 (campaign #83) — regression additions for the run22 closure
...
console.log('\n--- D156b [RESIDUAL]: the completion clause is the SUBJECT of the matrix predicate');
for (const [t, cur] of [
  ['No log exists and ACME was archived cannot be confirmed.', true],
  ['No record and Beta Corp was archived is unproven.', false],
  ['Nothing was found and ACME was archived remains unverified.', true],
  ['No entry survived and Beta Corp was deleted is unconfirmed.', true]])
  belt('RESIDUAL', 'D156b.subjectNP.' + JSON.stringify(t.slice(0, 46)), t, cur,
    'the completion clause is the grammatical SUBJECT of the matrix predicate ("<clause> cannot be confirmed"); ' +
    'recognising that needs subject-NP parsing, beyond a lexical belt. Pre-existing at every SHA (same shape as ' +
    '"There is no record, so ACME was archived is not something I can confirm."), deferred — not introduced here');
...
bind('CONTRACT', 'issue5.absentActionType.bareVerbLabel', 'restore it', [{ id: 'a', label: 'Restore', entityType: 'company' }], null, null);
{
  ...
  check('CONTRACT', 'sourceInvariant.d152ArmInlineNoNewConst',
    /\(\?:I\|We\|i\|we\)/.test(beltSlice)
      && !/\n\s*const\s+(?:ACTIVE_VOICE|ACTIVE_COMPLETION|ACTIVE_PAST)\b/.test(beltSlice),
    'run23/D155 fallback 1: the active-voice arm is an INLINE case-sensitive regex in readsAsCompletion, never a NEW const (a new const breaks run15/16/17/18 named-const assembly)');
}
