...
console.log('v66_object_shape_tiers_contract — source: ' + INDEX + '\n');
...
// ============================================================================================
// 9. CONTRACT (V66-D5) — MODEL-AUTHORED intent kind is validated at the boundary.
//
// Removing the ['mutation','confirmation','read','other'] allow-list survives every suite, because the
// only two consumers compare against 'mutation' and 'confirmation' and an unknown string fails both. So
// this is not a live behaviour today — it is an INPUT-VALIDATION BOUNDARY, and that is worth pinning on
// its own terms: `modelIntent.kind` is a field the MODEL writes, and the rule is that model-authored data
// is constrained where it enters, not wherever someone later remembers to check it. Pinned so that the
// next consumer to test `modelIntentKind === 'read'` or `!== null` inherits a validated value.
// ============================================================================================
{
  const kindOf = (kind) => {
    const at = SRC.indexOf('const modelIntentKind');
    const end = SRC.indexOf(';', at) + 1;
    return new Function('modelIntent', stripTS(SRC.slice(at, end)) + '\nreturn modelIntentKind;')({ kind });
  };
  for (const k of ['mutation', 'confirmation', 'read', 'other']) {
    check('CONTRACT', 'a declared intent kind is accepted: ' + JSON.stringify(k), kindOf(k) === k);
  }
  for (const k of ['banana', 'MUTATION', 'delete', '', 'proto']) {
    check('CONTRACT', 'an undeclared model-authored intent kind is rejected at the boundary: ' + JSON.stringify(k),
      kindOf(k) === null,
      'the model must not be able to introduce a kind the code has never heard of');
  }
  check('CONTRACT', 'a non-string kind is rejected', kindOf(42) === null && kindOf(null) === null);
}

// ============================================================================================
// 10. CONTRACT (V66-D6, F1) — the object test's boundary can match after a closing quote.
//
// STRONG_OBJECT ended in , which needs a word character on one side and therefore could NEVER match
// after a closing quote: `archive "Nomin Holding" then …` failed the object bar purely because the name
// was quoted. The structural fix (the intent tier consuming the executor's outcome) covers the commands
// the executor RESOLVES, so this boundary is only load-bearing for the ones it does not — which is
// exactly the shape that goes untested and then rots. Pinned directly on the sliced regex.
// ============================================================================================
{
  ...
  for (const obj of ['a number to each company and list them', 'and tell me what is left', '']) {
    check('CONTRACT', 'STRONG_OBJECT still rejects a non-object: ' + JSON.stringify(obj), !SO.test(obj),
      'the negative half — a bar that accepts everything is not a bar');
  }
}

// ============================================================================================
// 11. CONTRACT (V66-D6, F2) — the tier-boundary value is really COMPUTED from the executor's outputs.
//
// Every other row here INJECTS commandFallbackResolvedVerb, which is right for testing what the intent
// tier does with it — but it means the source could stop computing the value entirely and those rows
// would not notice. The mutation proof found exactly that (m1_executor_outcome_never_computed survived
// four suites). A value the harness supplies is a value the harness cannot vouch for, so the declaration
// itself is sliced and executed here.
// ============================================================================================
{
  const at = SRC.indexOf('const commandFallbackResolvedVerb');
  if (at < 0) throw new Error('v66: commandFallbackResolvedVerb not found — the tier boundary is gone');
  const decl = stripTS(SRC.slice(at, SRC.indexOf(';', SRC.indexOf('            : null', at)) + 1));
  const verbOf = (allowed, arch, rest, mArch = false, mRest = false) => new Function(
    'commandFallbackAllowed', 'archiveCompanyIds', 'restoreCompanyIds', 'modelEmittedArchive',
    'modelEmittedRestore', decl + '\nreturn commandFallbackResolvedVerb;',
  )(allowed, arch, rest, mArch, mRest);
  check('CONTRACT', 'the boundary value is archive when the executor resolved an archive target',
    verbOf(true, ['c1'], []) === 'archive',
    'the intent tier is handed a fact about THIS turn; a constant would make every row above vacuous');
  check('CONTRACT', 'the boundary value is restore when the executor resolved a restore target',
    verbOf(true, [], ['c1']) === 'restore');
  check('CONTRACT', 'the boundary value is null when the executor resolved nothing',
    verbOf(true, [], []) === null,
    'the negative half — a constant "archive" would arm the receipt on every turn');
  check('CONTRACT', 'the boundary value is null when the gate refused',
    verbOf(false, ['c1'], []) === null);
  check('CONTRACT', 'the boundary value is null when the MODEL emitted the lifecycle field',
    verbOf(true, ['c1'], [], true) === null && verbOf(true, [], ['c1'], false, true) === null,
    'a model-emitted lifecycle field is the structured path, which arms the receipt by itself');
}

// ---------------------------------------------------------------- result
console.log('\nv66_object_shape_tiers_contract: ' + pass + ' passed, '
