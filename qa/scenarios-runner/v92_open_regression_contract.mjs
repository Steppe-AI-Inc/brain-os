...
function buildDecision(src) {
  ...
  return (turn) => {
    ...
    return fn(t.verifiedClaims, t.model, t.groundedOutcomeThisTurn, t.claimsFutureActionWithNoPlan,
      { summary: t.summary, pendingAction: t.pendingAction ?? null, claims: t.rawClaims }, t.rawClaims, t.deterministicPrefix, t.claimExecutionEvidence, t.hasRejectedClaims, t.requestedIntent ?? { verb: 'archive', field: null });
  };
}
...
console.log('=== v92 open-regression contract (promoted from verifier #30) — source ' + SRC);
...
// belt from a NAMED-CONST LIST, so a new TOP-LEVEL declaration in the block is silently dropped by
// them. That is the hazard, and it is what verifier #30's own prepared fix hit when it declared `nx`
// at belt top level and crashed run15.
//
// Narrowed in run30 from a flat pinned sequence over the whole block to top-level declarations only.
// The original already whitelisted n/m/rel/p as "locals inside completionIsNegated"; locals are not
// the hazard, because completionIsNegated is extracted brace-balanced and its body travels with it.
// run30's fix adds scan/mm/nameInternal/titleHead/ppInternal there and every extractor-based suite
// stays green, which is the evidence. The coverage assertion below keeps this honest by proving the
// narrowed detector still catches the top-level case it exists for.
const topLevelDecls = (src) => {
  const blk = stripCommentLines(beltBlock(src));
  let depth = 0, out = [], tok = /[{}]|\b(?:const|let)\s+([A-Za-z_$][\w$]*)/g, m;
...
{
  // `me` is not a real declaration: it is a lexical artifact of the `let me (?:archive|…)` alternation
  // inside EXECUTION_IN_PROGRESS's STRING. Pinned deliberately, because the extraction is what the
  // suites actually do.
  const EXPECTED = ['LEGACY_PAST_COMPLETION', 'PROGRESS_VERBS', 'EXECUTION_IN_PROGRESS', 'me',
  ...
  const declared = topLevelDecls(TEXT);
  check('CONTRACT', 'belt declares exactly the known TOP-LEVEL const list (no new declaration)',
    declared.join(',') === EXPECTED.join(','), 'got: ' + declared.join(','));
}
{
  // COVERAGE: the narrowed detector must still catch a new TOP-LEVEL declaration in the belt block.
  // Mutate the source in memory the way verifier #30's rejected fix did, and require detection.
  const mutated = TEXT.replace('const readsAsCompletion =', 'const nx = 1;\n        const readsAsCompletion =');
  const before = topLevelDecls(TEXT).join(',');
  const after = mutated === TEXT ? before : topLevelDecls(mutated).join(',');
  check('CONTRACT', 'COVERAGE: a new TOP-LEVEL belt declaration is still detected (non-vacuous)',
    mutated !== TEXT && after !== before && after.includes('nx'),
    'injecting a top-level const did not change the detected list — this contract would not catch the thing it exists for');
}
{
...
// ── The 5 v92-differential regression classes verifier #30 opened, now CLOSED by run30 and
// pinned so they stay closed. Each shape is one deployed v92
...
if (failures.length) {
  console.log('\nRED — the v92-differential deployment gate is not satisfied:');
