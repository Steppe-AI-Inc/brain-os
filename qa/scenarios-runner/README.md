...
| `sc081_create_own_company_full_isolation.sql` | SC-081 | `create_own_company()` (migration `202609010001`) — creator becomes owner-only of the new company, employer membership untouched, no subsidiary relationship, no manager authority gained at employer; unrelated employee/employer-manager/unrelated-company-user all get 0 rows across company/membership/people/projects/tasks/goals/memories/documents. Run LIVE against production, not rolled-back-and-untested — KNOWN_FAILURE_MODES.md #58 | 2026-09-01 |
| `sc081_anon_persona_isolated.sql` | SC-081 | anon gets 0 rows on `tasks` for a freshly-created personal company (querying `companies` directly hits an unrelated pre-existing gap — see #58) | 2026-09-01 |
| `create_own_company_live_state_check.sql` | (new fix, no SC-#) | reusable live check that `create_own_company()`'s function body + grants are genuinely applied in production, not just recorded in migration history — KNOWN_FAILURE_MODES.md #59 | 2026-09-01 |
| `sc082_manager_column_cross_company_isolation.sql` | SC-082 | People page Manager column (`person_assignments`, per-organization) — fixture-based cross-company isolation proof since production had zero real manager data to observe passively — KNOWN_FAILURE_MODES.md #59 | 2026-09-01 |
| `anon_companies_investor_viewer_permission_denied_gap.sql` | (new finding, no SC-#) | documents the pre-existing gap (anon crashes instead of getting an empty result on 5 tables that reference `is_investor_viewer_of()`) — KNOWN_FAILURE_MODES.md #58/#59. See `is_investor_viewer_of_anon_grant_fix.sql` for the prepared fix's own proof | 2026-09-01 |
| `is_investor_viewer_of_anon_grant_fix.sql` | (new fix, no SC-#) | adversarial rollback proof for `202609010002_fix_investor_viewer_anon_rls_helper_grant.sql` (GATED, not yet pushed) — temporary in-transaction GRANT, all 5 tables return a clean empty result for anon, direct enumeration with real+fake company UUIDs proves zero data exposure, confirmed zero residue after rollback — KNOWN_FAILURE_MODES.md #60 | 2026-09-01 |
...
| `agent_run_capacity_retry_claim_security.sql` | (new finding, no SC-#) | **NOT YET EXECUTED — no DB transport in the authoring session.** Acceptance criteria for `202609030001_agent_run_capacity_retry.sql` (`claim_blocked_run_for_retry`) and the four DB-side defects in KNOWN_FAILURE_MODES.md #62: attempt cap unenforced in SQL (D1), unclassified failures claimable (D2), `claimed_by` never reset so a run is recoverable only once (D3), the supervisor's own superuser transport is denied by `is_founder_or_admin()` and misreported as "migration not applied" (D4), and a company manager being able to forge every retry input the supervisor consumes (D5). Inlines the migration DDL so it runs before or after the push | NOT RUN (authored 2026-09-03) |
