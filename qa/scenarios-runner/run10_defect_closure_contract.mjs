// RUN10 DEFECT CLOSURE CONTRACT — verifier #10's cases (campaign #70), promoted with
// FIXED expectations per that file's own instruction. One deliberate adaptation,
// disclosed: D79.taskTitle expects the QUOTED render — the run10 design keeps an
// assertion-shaped name's identity by framing it as a title (the lifecycle-line
// precedent), rather than rendering it bare; compound aux-assertions still collapse
// (R10.label.canonical). Original file preserved at qa/verification/proposed/.
...
const SRC = process.env.SEM_INDEX_SRC || resolve(dirname(fileURLToPath(import.meta.url)), '../../supabase/functions/sem-ai-command/index.ts');
...
// Exit guard (restored on promotion — a suite that reports failures with exit 0 is
// decorative; the mutation battery relies on the exit code).
