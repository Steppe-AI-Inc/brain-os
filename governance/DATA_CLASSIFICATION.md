...
| `agent_runs` retry/checkpoint columns incl. `execution_mode` (PREPARED, 202609030001) | confidential (founder/admin, or the supervisor's direct-connection identity) | `guard_agent_run_retry_columns` (SECURITY DEFINER, tests `session_user`, 26 columns incl. execution_mode — round 3 D-3); `claim_blocked_run_for_retry` holds NO EXECUTE grant (direct-connection-only, round 3 D-2) | personas under session_user = qa_authenticator (round 3 D-1); SKIP LOCKED double-claim under two real connections (concurrency.mjs) |
| `chat_channels.created_by_profile_id` (PREPARED, 202609040001 — sequenced with C) | internal — but it is an OWNERSHIP PREMISE another migration's gate trusts, so it is now immutable except to founder/admin (`chat_channels_creator_immutable_guard`, round 5 R5-1) | the guard trigger + `chat_channels_write_scope` | PGlite personas C.R5-1; needs the real-PostgreSQL job |
