-- KNOWN_FAILURE_MODES.md #7: company_id was never populated on work_orders/chat_channels/
-- audit_logs (100% null across every real row, confirmed live) -- not a leak (a non-manager
-- test account's visible rows matched its own actor/creator rows exactly), but it makes the
-- is_company_manager(company_id) RLS branch on these tables inert: a real company_manager
-- currently cannot see any audit trail, work order, or chat history for their own company,
-- because nothing ever sets company_id at creation time.
--
-- sem-ai-command now derives a primary company id in TypeScript -- only when the command is
-- unambiguously about one company (the active channel's own company_id, or every task this
-- command touched agreeing on the same company_id) -- and passes it through as a new
-- parameter. Never guessed when multiple companies are involved or none are; a wrong company
-- tag would be worse than leaving it null, so ambiguous cases stay null exactly as before.

drop function if exists public.sem_execute_ai_command(text, jsonb, jsonb, int, jsonb, jsonb, text, int, int, numeric, uuid[], jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, uuid, jsonb);
...
-- understanding once the stream finishes" pattern (chat-client.tsx already does this for
-- title) -- the channel is created before the model responds, so its company can only be
-- known afterward. RLS-scoped exactly like a normal channel update; not a bypass.
