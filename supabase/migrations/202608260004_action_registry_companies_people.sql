-- Chat/UI action parity, step 1: let sem-ai-command create real companies/people rows,
-- not just tasks that describe the work. Founder tested this directly (pasted a real
-- description of SEM Technologies LLC / Steppe AI / SEM Mongolia / SEM GRT and the
-- current team) and got 8 well-reasoned tasks but zero actual companies/people rows —
-- expected given the prior schema had no such action at all. This extends the existing
-- transactional RPC with two more typed arrays, same insert-loop-and-collect-ids pattern
-- already used for tasks/approvals/deletions, running under `security invoker` so the
-- existing companies_write_admin (founder/admin only) and people_write_manager
-- (company-manager+) RLS policies gate chat exactly the same as manual UI already is —
-- no new policy needed, real parity for free.
