-- Chat/UI action parity, step 2: create_project and create_goal, same pattern as step 1
-- (202608260004, createCompanies/createPeople). Both projects.company_id and
-- goals.company_id are NOT NULL in the schema, so each entry must resolve to a real
-- company id or a companyIndex into companies created earlier in the same request —
-- same resolution already used for people. If neither resolves, the insert raises a
-- normal not-null-violation error surfaced to the user, same as any other bad RPC call
-- today; not silently swallowed.
