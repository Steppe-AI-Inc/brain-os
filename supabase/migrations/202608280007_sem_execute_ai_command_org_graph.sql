-- Full replacement of sem_execute_ai_command: routes 'current' company-to-company
-- relationships through the new idempotent set_company_relationship() RPC
-- (202608280006), wraps that call so a cycle/ownership-limit violation skips just that
-- one relationship instead of aborting the founder's entire chat command, and lets
-- createCompanies set organization_type directly. See 202608280006 and the
-- sem-ai-command/index.ts changes in the same commit for the full story.
