...
SEM.Modules.productionCore = (()=>{
  ...
  return {
    render(){ const s=SEM.Store.get(); const sess=SEM.DataService.getSession(); return `
      ...
      <div class="grid grid2" style="margin-top:18px">
        <div class="card"><h3>Cloud configuration</h3><p class="muted">Configure Supabase after running <code>supabase/schema-v0.7-production-core.sql</code>. This page tests the real backend path without rewriting the UI.</p>
          ...
          <div class="formGroup"><label>AI Native Chat backend</label><select id="chatOpsBackend"><option value="local">Local simulation — zero API tokens, nothing persisted to Supabase</option><option value="supabase">Supabase Edge Function — real, RLS-scoped, persists tasks/approvals server-side</option></select></div>
          <p class="muted">Requires Supabase to be configured and signed in above. If a command fails against the Edge Function (not configured, not signed in, or a server error), ChatOps shows the error instead of silently falling back to local simulation.</p>
          <div class="actions"><button class="primary" id="saveChatOpsBackend">Save chat backend</button></div>
        </div>
    ...
    afterRender(){
      U.$('#chatOpsBackend').value=SEM.Store.get().settings.chatOpsBackend||'local';
      U.$('#saveChatOpsBackend').onclick=()=>{ const s=SEM.Store.get(); s.settings.chatOpsBackend=U.$('#chatOpsBackend').value; SEM.Store.save(); U.toast(`AI Native Chat backend set to ${s.settings.chatOpsBackend}`); };
