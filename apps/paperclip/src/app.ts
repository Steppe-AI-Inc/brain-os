/**
 * Paperclip — orchestrator + dashboard. App factory.
 *
 * Split from index.ts so a serverless entrypoint (api/index.ts) can build
 * the same Fastify instance without calling `.listen()`. `index.ts` (local /
 * Docker) still owns the process lifecycle: it calls `buildApp()`, then
 * listens and wires signal-based shutdown.
 */
...
export async function buildApp(): Promise<FastifyInstance> {
  // Fail-fast on missing required env (Portkey keys, etc.) before serving
  // any request. Clear error message > silent runtime null returns.
  requireConfig();
  ...
  if (config.skipBootTasks) {
    app.log.info(
      "boot tasks skipped (PAPERCLIP_SKIP_BOOT_TASKS=true) — run `npm run migrate` once " +
        "against this database before relying on this in a serverless deploy",
    );
    return app;
  }
